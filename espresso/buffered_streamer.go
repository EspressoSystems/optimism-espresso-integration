...
func (b *BufferedEspressoStreamer[B]) RefreshSafeL1Origin(safeL1Origin eth.BlockID) error {
	if safeL1Origin.Number < b.startingBatchPos {
		...
		b.readPos = 0
		b.startingBatchPos = safeL1Origin.Number
		b.batches = make([]*B, 0)
	...
	if safeL1Origin.Number > b.startingBatchPos {
		...
		nextSafePosition := safeL1Origin.Number
		positionAdjustment := nextSafePosition - b.startingBatchPos
