...
func CombineDeployConfig(intent *Intent, chainIntent *ChainIntent, state *State, chainState *ChainState) (genesis.DeployConfig, error) {
	...
	// when no offset has been provided. The BatchAuthenticator is deployed at genesis and
	// the batcher posts via authenticated events from an EOA distinct from
	// SystemConfig.batcherHash; the verifier must therefore derive in post-fork
	// (event-based) mode from block 0. Production chains that want to stage Espresso onto
	// an existing chain set L2GenesisEspressoEnforcementTimeOffset explicitly upstream of
	// this function and that explicit value is preserved here.
	if chainIntent.EspressoEnabled && upgradeSchedule.L2GenesisEspressoEnforcementTimeOffset == nil {
