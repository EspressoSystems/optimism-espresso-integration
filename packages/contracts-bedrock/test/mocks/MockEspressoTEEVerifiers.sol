...
contract MockEspressoNitroTEEVerifier is IEspressoNitroTEEVerifier {
    ...
    function nitroEnclaveVerifier()
        external
        pure
        override
        returns (INitroEnclaveVerifier)
    {
...
contract MockEspressoTEEVerifier is
    ...
    IEspressoNitroTEEVerifier
{
    ...
    function isSignerValid(
        ...
        IEspressoNitroTEEVerifier nitroVerifier = _useExternalNitroVerifier
            ? _nitroVerifier
            : IEspressoNitroTEEVerifier(address(this));
    ...
    function registerService(
        ...
        require(
            teeType == TeeType.NITRO,
            "MockEspressoTEEVerifier: only NITRO supported"
        );
        if (attestation.length >= 20) {
            address signer = address(uint160(bytes20(attestation[:20])));
    ...
    function setEspressoNitroTEEVerifier(
        IEspressoNitroTEEVerifier verifier
    ) external override {
    ...
    function setNitroEnclaveVerifier(
        address
    ) external override(IEspressoNitroTEEVerifier, IEspressoTEEVerifier) {}
