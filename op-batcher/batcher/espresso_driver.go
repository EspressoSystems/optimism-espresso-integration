...
// authGroup is bounded to this many concurrent BatchInbox transactions
// simultaneously waiting on a BatchAuthenticator transaction (both for the
// fallback and the TEE path).
const authGroupLimit = 128

// initAuthGroup applies the concurrency limit. Called from NewBatchSubmitter.
func (l *BatchSubmitter) initAuthGroup() {
	l.authGroup.SetLimit(authGroupLimit)
}
