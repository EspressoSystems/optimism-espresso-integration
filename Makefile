...
lint-go: ## Lints Go code with specific linters (espresso only; op-service/op-node/op-chain-ops have typecheck errors).
	golangci-lint run -E goimports,sqlclosecheck,bodyclose,asciicheck,misspell,errorlint --timeout 10m ./espresso/...
	go mod tidy -diff
.PHONY: lint-go

lint-go-fix: ## Lints Go code with specific linters and fixes reported issues
	golangci-lint run -E goimports,sqlclosecheck,bodyclose,asciicheck,misspell,errorlint --timeout 10m ./espresso/... --fix
.PHONY: lint-go-fix
