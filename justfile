...
devnet-TestBatcherRestart-test: build-devnet
  U_ID={{uid}} GID={{gid}} go test -timeout 30m -p 1 -count 1 -v -run TestBatcherRestart ./espresso/devnet-tests/...

devnet-TestEnclaveRestart-test: build-devnet
  U_ID={{uid}} GID={{gid}} ESPRESSO_RUN_ENCLAVE_TESTS=true go test -timeout 30m -p 1 -count 1 -v -run TestEnclaveRestart ./espresso/devnet-tests/...
